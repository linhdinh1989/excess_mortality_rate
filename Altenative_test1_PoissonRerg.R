...
## Model flu --
mod.flu1 =glm(dealth.flu ~ educa + gage  + nperiod + square + cos1 + sin1 + cos2 +sin2,
             data = dtset,
             offset = log(persons.year),
             family=poisson(link="log"))

summary(mod.flu1)
predicted <- predict(mod.flu1, type = 'link', se.fit=T)
dtset$expected.flu <- exp(predicted$fit)
dtset$upr.flu <-  exp(predicted$fit+predicted$se.fit*qnorm(0.995))

## Modela all death --
mod.all1 =glm(dealth.all ~ educa + gage  + nperiod + square + cos1 + sin1 + cos2 +sin2,
              data = dtset,
              offset = log(persons.year),
              family=poisson(link="log"))
...
### Predictions made by age group and educational level
### must be added for all age groups and educational levels
### to define epidemic thresholds
cat(names(dtset))
...
pdf(file='graph/Altenative_test1_PoissonRerg.pdf', width = 8.3,  height =  11.7)
...
dtset2[epi.flu==TRUE, ':='(dealth.flu=NA, dealth.all=NA)]  
...
for (i in levels(dtset2$gage)) {
  for (j in  levels(dtset2$educa) ){
    ...
    model =glm(dealth.flu ~ nperiod + square + cos1 + sin1 + cos2 +sin2,
...
for (i in levels(dtset2$gage)) {
  for (j in  levels(dtset2$educa) ){
    ...
    model =glm(dealth.all ~ nperiod + square + cos1 + sin1 + cos2 +sin2,
               data = dtset2[gage==i & educa == j],
               offset = log(persons.year),
               family=poisson(link="log"))
...
dtset2[,excess2.flu:= ifelse(epi.flu              & 
                             winter.months & 
                             dealth.flu0 > upr2.flu, dealth.flu0-upr2.flu,0)]
dtset2[,excess.all:= ifelse(epi.flu              & 
                                   winter.months &
                                   dealth.all0 > upr.all, dealth.all0-upr.all,0)]
dtset2[,excess2.all:= ifelse(epi.flu              & 
                             winter.months &
                             dealth.all0 > upr2.all, dealth.all0-upr2.all,0)]

## Expected and observed graph
melt(dtset2, 
     id.vars=1:6, 
